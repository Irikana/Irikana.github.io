...
- callout 的图标位统一使用排版符号 ※（U+203B 参考记号，属文字记号不是 emoji）；历史上用过的 ℹ️ 及其余表情图标已在 alpha-023 全部替换为 ※，不再作为例外保留
...
## 移动端浮层编排契约（alpha-022 起）

### 原则
- **移动端（≤768px）不得仅缩小 PC 端浮层**：凡冻结顶栏内已能进入的功能，浮层一律撤走，不占正文
- 浮层重新编排为**左右两条竖列并上下错开**，禁止两个浮层落在同一坐标区间（历史缺陷：主题钮与回到顶部同为 right:8 bottom:68，后者在手机上不可点）
- 展开型面板（目录、收藏、阅读工具、搜索结果）一律**贴边 + 限高（vh）+ overflow-y: auto**，不得以固定像素宽压在小屏正文中段

### 移动端处置清单（规则位于 css/library-refit.css 的「移动端浮层编排」段）
| 构件 | 移动端处置 | 替代入口 |
|------|-----------|---------|
...
| 收藏面板 / 搜索结果区 | 保留，贴边限高（42vh / 46vh）可滚 | 同上 |
